...
module.exports = {
  ...
  jira: {
    ...
    opsConfig: {
      ...
      getProjectByKey:      { desc: 'Get a project by its key', args: ['projectKey'], params: { projectKey: '' } },
      getIssues:            { desc: 'Fetch issues with optional JQL filter', params: { projectKey: '', maxResults: 10 } },
      getIssueByKey:        { desc: 'Get a single issue by its key', args: ['issueKey'], params: { issueKey: '' } },
      createIssue:          { desc: 'Create a new issue', params: { projectKey: '', summary: '', issueType: 'Bug', description: '', priority: 'Medium' } },
      updateIssue:          { desc: 'Update an issue by key', args: ['issueKey'], params: { issueKey: '', summary: '', priority: 'Medium' } },
      deleteIssue:          { desc: 'Delete an issue by key', args: ['issueKey'], params: { issueKey: '' } },
      bulkCreateIssues:     { desc: 'Bulk create multiple issues', args: ['requests'], params: { requests: [] } },
      getComments:          { desc: 'Get all comments on an issue', args: ['issueKey'], params: { issueKey: '' } },
      addComment:           { desc: 'Add a comment to an issue', args: ['issueKey', 'body'], params: { issueKey: '', body: '' } },
      getTransitions:       { desc: 'Get available status transitions for an issue', args: ['issueKey'], params: { issueKey: '' } },
      transitionIssue:      { desc: 'Change the status of an issue', args: ['issueKey', 'transitionId'], params: { issueKey: '', transitionId: '' } },
      getSprints:           { desc: 'Get sprints for a board', args: ['boardId'], params: { boardId: 0 } },
      getActiveSprint:      { desc: 'Get the active sprint for a board', args: ['boardId'], params: { boardId: 0 } },
      createSecurityTicket: { desc: 'Create a security finding ticket', args: ['projectKey', 'title', 'description', 'severity', 'source'], params: { projectKey: '', title: '', description: '', severity: 'high', source: 'playwright-test' } },
    },
  },
